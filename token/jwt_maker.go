...
func NewJWTMaker(secretKey string) (Maker, error) {
	if len(secretKey) < minSecretKeySize {
		return nil, fmt.Errorf("invalid key size : must be at lease %d character", minSecretKeySize)
	}
	return &JWTMaker{secretKey}, nil
}

// CreateToken creates new token for specific username and duration
func (maker *JWTMaker) CreateToken(username string, duraion time.Duration) (string, error) {
